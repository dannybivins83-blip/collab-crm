...
- **Git tip (verified 2026-06-30):** `f5a3162` on `main`. ⚠️ **DEPLOY PIPELINE FACT (verified via Render API):** the `collab-crm` Render service deploys from **`agent/gc-consolidation`**, NOT `main`. `main` had drifted **119 commits ahead** and the live site was stale at `2162203` (2026-06-26) — i.e. several sessions' "live" work was never actually deployed. Fixed by fast-forwarding `agent/gc-consolidation` → `main` (`git push origin main:agent/gc-consolidation`); Render rebuilt and `f5a3162` is confirmed **live**. **To deploy: push (or FF) `agent/gc-consolidation`** — pushing `main` alone does nothing. Recommend pointing Render at `main` to end the drift.
