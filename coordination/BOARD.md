...
- **Git tip (verified 2026-06-30):** `1e48dfa` on `main`, confirmed **live** via Render API. ✅ **DEPLOY PIPELINE FIXED:** the `collab-crm` Render service deploy branch was changed from the frozen `agent/gc-consolidation` (stale at `2162203`, 4 days/119 commits behind — earlier "live" claims were false) to **`main`** via the Render API. Pushing `main` now auto-deploys (verified: `e4bb608` + `1e48dfa` both deployed straight from main). `RENDER_API_KEY` is in the env — use `GET /v1/services/srv-d8kq47jbc2fs73crtnug/deploys` to confirm any deploy reached `live`.
...
| 🛒 RoofPortal Sales | overlord/crm-ui (2146f22a session) | `templates/portal_landing.html`, `modules/demos.py` landing routes, `portal_leads` table | ✅ LIVE 8/7: myroofportal.com root = sales landing + pricing ($499+$149/mo) + lead form (verified prod POST); demo untouched at /demo/roof-portal. Pushed `673f92e` to BOTH main + agent/gc-consolidation (note: a gc-consolidation push visibly deployed 8/7 — CLAUDE.md says main-only since 6/30; someone verify Render's actual branch config). Campaign: FL CCC list (10,750) pulled; DBPR email records request drafted (owner to send); $25K acquisition one-pager built. Also: `db.py` TABLE_ALLOWLIST += portal_leads; demo portal restyled to owner mockup (supersedes Ascent look on demo); 8/12 owner-review fixes: mobile-responsive (375px clean, vertical tracker) + sticky-rail desktop grid (`8850e31`) |
