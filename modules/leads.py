...
def parse_zip():
    ...
    if not api_key:
        return jsonify({"ok": False, "error": "ANTHROPIC_API_KEY not set — add it to Render env"}), 503
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "Not a valid ZIP file — please upload a .zip archive."}), 400
...
def parse_image():
    ...
    api_key = config.ANTHROPIC_API_KEY
    if not api_key:
        return jsonify({"ok": False, "error": "ANTHROPIC_API_KEY not set — add it to your .env or Render env"}), 503
...
def delete(lead_id):
    ...
    db.execute("DELETE FROM appointments WHERE lead_id=?", (lead_id,))
